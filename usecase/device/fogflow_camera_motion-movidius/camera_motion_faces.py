...
# indicating its box and direction. All parameters must be informed in camera_motion_faces.json
...
class CameraMotion:
    ...
    def __init__(self, model, target, confidence, camera_id, direction, src_video, roi=None, max_width=500):
        # loading the deep learning model
        self.net = cv2.dnn.readNet(model, model.replace('.xml', '.bin'))
        self.net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU if target == 'cpu' else cv2.dnn.DNN_TARGET_MYRIAD)
...
def thread_function(name):
    ...
    camera = CameraMotion(camera_id=profile['id'], src_video=profile['source'], roi=profile['roi'],
                          ...
                          model=profile['model'], target=profile['target'], confidence=profile['confidence'])
    camera.run()
